...
private:
	...
	string _PrepareTransferLogRecord(clsBankClient Client2, float Amount, string Seperator = "#//#")
	{
	...
	void _RegisterTransferLog(clsBankClient Client2, float Amount)
	{
		fstream MyFile;
		MyFile.open("TransferLog.txt", ios::out | ios::app);
		if (MyFile.is_open())
		{
			MyFile << _PrepareTransferLogRecord(Client2, Amount) << endl;
			MyFile.close();
		}

	}
	struct stTransferLogRecord;
	static stTransferLogRecord _ConverTransferLogLineToRecord(string Line, string Seperator = "#//#")
	{
		vector < string>vTransferLog = clsString::Split(Line, Seperator);
		stTransferLogRecord TransferLog;
		TransferLog.TimeTransfer = vTransferLog[0];
		TransferLog.sClient = vTransferLog[1];
		TransferLog.dClient = vTransferLog[2];
...
public:
	...
	static vector<stTransferLogRecord> GetTransferLogList()
	{
		...
		if (MyFile.is_open())
		{
			...
			while (getline(MyFile, Line))
			{
				TransferLog = _ConverTransferLogLineToRecord(Line);
